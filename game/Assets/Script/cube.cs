	...
	public bool CheckPosition(Vector3 input_pos, out Vector3 pos)
	{
		...
		foreach (State state in states) 
		{
			...
			if (state.IsFliped == is_flip) 
			{
				...
				//Debug.Log ("X : " + pos.x);
				//Debug.Log ("Y : " + pos.y);
				//pos.x = pos.x / (RuntimeManager.resolution.Width *( Screen.width-1000));
				//Debug.Log (RuntimeManager.resolution.Width);
				//Debug.Log (Screen.width);
				pos.y = Screen.height - pos.y / RuntimeManager.resolution.Height * Screen.height;

				 Debug.Log(name+"X : " + pos.x + " Y : " + pos.y + " F : " + state.IsFliped);
				Debug.Log("Cube ：X : " + input_pos.x+ " Y : " + input_pos.y + " F : " + is_flip);
				// Debug.Log(Vector3.Distance(Input.mousePosition, pos));
				if (Vector2.Distance (new Vector2(input_pos.x, input_pos.y), new Vector2(pos.x, pos.y)) <= 150) 
				{
					count++;
					Debug.Log (count);
					return true;
                ...
                else
                {
					...
					if (CheckPosition(Input.mousePosition, out pos)) 
					{
						...
						if (CheckCompleted ()) {
                            ...
                            playboard.transform.Find("poetry").GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("poetry/poetry_" + RuntimeManager.scene.Replace('.', '_'));
